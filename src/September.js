...
const September = () => {
  ...
  useEffect(() => {
    const fetchSeptemberVideos = async () => {
      try {
        const q = query(
        ...
        console.error('Error fetching September recordings:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchSeptemberVideos();
  }, []);
    ...
    <div className="flex flex-col min-h-screen bg-gray-100">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <h1 className="text-2xl font-bold text-gray-800 font-serif">Watch Recordings - September</h1>
        </div>
      </header>
      ...
      <main className="flex-grow px-6 py-10 max-w-7xl mx-auto">
        {loading ? (
          ...
          <p className="text-center text-gray-600">No recordings found for September.</p>
        ) : (
