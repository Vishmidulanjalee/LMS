...
const UploadRecordings = () => {
  ...
  const [recordType, setRecordType] = useState(''); // optional
  ...
  const fetchRecordings = async () => {
    ...
    snapshot.docs.forEach(docSnap => {
      ...
      const typeLabel = (data.type && data.type.trim()) ? data.type : 'No Type';
      const key = `${typeLabel} - ${m}`;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !link || !thumbnail || !month) {
      alert("Title, link, month, and thumbnail are required.");
      return;
    }
    try {
      const payload = {
      ...
      if (recordType && recordType.trim()) payload.type = recordType.trim(); // only add if provided
  ...
  const handleDelete = async (id) => {
    ...
    catch (e) { console.error(e); alert("Failed to delete recording."); }
  };

  const startEdit = (rec) => { setEditingId(rec.id); setEditingTitle(rec.title || ''); };
        ...
        <div className="bg-white shadow-md rounded-lg p-6 lg:w-1/2">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <input
              ...
              className="border border-gray-300 rounded p-2" placeholder="Enter video title" required
            />

            <label className="text-gray-700 font-medium">YouTube Link</label>
            <input
              type="url" value={link} onChange={(e) => setLink(e.target.value)}
              className="border border-gray-300 rounded p-2" placeholder="https://youtube.com/..." required
            />

            <label className="text-gray-700 font-medium">Record Type (optional)</label>
            <select
              value={recordType} onChange={(e) => setRecordType(e.target.value)}
              className="border border-gray-300 rounded p-2"
            >
              <option value="">— None —</option>
              {recordTypes.map((t, i) => <option key={i} value={t}>{t}</option>)}
            </select>
