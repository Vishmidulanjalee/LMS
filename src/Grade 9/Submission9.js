...
const Submission9 = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `studenthomework9/${fileType}/${file.name}`);
      await uploadBytes(storageRef, file);

      const fileURL = await getDownloadURL(storageRef);

      // Save to Firestore
      await addDoc(collection(db, 'studenthomework9'), {
        name,
        fileURL,
        className,
        institute,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Submit Your Homework - Grade 9</h2>
