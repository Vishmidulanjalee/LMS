...
const Apara = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const q = query(collection(db, 'documents'), where('category', '==', 'A පාර Papers'));
      ...
      <header style={{ background:'white',borderBottom:'1px solid #E5E7EB',position:'sticky',top:0,zIndex:10 }}>
        ...
        <div style={{ padding:'15px 32px',display:'flex',alignItems:'center',justifyContent:'space-between',maxWidth:1280,margin:'0 auto',flexWrap:'wrap',gap:12 }}>
          <div style={{ display:'flex',alignItems:'center',gap:14 }}>
            ...
            <div>
              <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:'#111827',margin:0,lineHeight:1.2 }}>A පාර Papers</h1>
              <p style={{ fontSize:13,color:'#9CA3AF',margin:'3px 0 0',fontWeight:400 }}>Selected past papers</p>
            </div>
          </div>
      ...
      <main style={{ flexGrow:1,padding:'30px 32px',maxWidth:1280,margin:'0 auto',width:'100%' }}>
        ...
        {!loading && documents.length === 0 && (
          <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'80px 0',gap:14 }}>
            ...
            <p style={{ fontSize:13,color:'#9CA3AF',margin:0 }}>Check back later for new A පාර Papers.</p>
          </div>
        )}
        {!loading && documents.length > 0 && (
