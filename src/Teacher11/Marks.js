...
const TeacherMarks11 = () => {
  ...
  useEffect(() => {
    const fetchMarks = async () => {
      const marksCollection = collection(db, "marks11"); // Updated collection for Grade 11
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const fileRef = ref(storage, `marks11/${file.name}`); // Updated storage path for Grade 11
      await uploadBytes(fileRef, file);
      const fileURL = await getDownloadURL(fileRef);

      const docRef = await addDoc(collection(db, "marks11"), { // Updated collection name for Grade 11
        fileName,
        fileURL,
        uploadedAt: new Date()
      });
  ...
  const handleDelete = async (id, fileURL) => {
    try {
      ...
      await deleteDoc(doc(db, "marks11", id)); // Corrected collection name
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Upload Marks - Grade 11</h2>
