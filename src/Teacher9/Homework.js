...
const TeacherHomework9 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const homeworkSnapshot = await getDocs(collection(db, 'homework9')); // Updated collection name for Grade 9
        ...
        const submissionSnapshot = await getDocs(collection(db, 'studenthomework9')); // Updated collection name for submissions
  ...
  const handleAddHomework = async (e) => {
    ...
    try {
      ...
      setUploading(true);
      if (file) {
        const fileRef = ref(storage, `homework9/${file.name}`); // Updated file path for Grade 9
      ...
      const docRef = await addDoc(collection(db, 'homework9'), newHomework); // Updated collection name for Grade 9
  ...
  const handleDeleteHomework = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this homework?')) {
      try {
        ...
        await deleteDoc(doc(db, 'homework9', id)); // Updated collection name for Grade 9
  ...
  const handleDeleteSubmission = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this submission?')) {
      try {
        await deleteDoc(doc(db, 'studenthomework9', id)); // Updated collection name for submissions

        if (fileURL) {
          const fileRef = ref(storage, `studenthomework9/${fileURL.split('/').pop()}`);
          await deleteObject(fileRef);
        }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Homework - Grade 9</h2> {/* Updated title for Grade 9 */}
...
export default TeacherHomework9;
