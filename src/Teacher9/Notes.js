...
const TeacherNotesGrade9 = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      setLoading(true);
      try {
        const querySnapshot = await getDocs(collection(db, 'notes9'));
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'notes9'), newNote);
  ...
  const grade9Notes = notesList.filter(note => note.grade === '9');
  ...
  const groupedNotesByInstitution = grade9Notes.reduce((acc, note) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Upload Notes - Grade 9</h2>
      ...
      <div className="mb-8">
        ...
        <p>No notes available for Grade 9.</p>
      )} 
    </div>
    </div>
  );
...
export default TeacherNotesGrade9;
