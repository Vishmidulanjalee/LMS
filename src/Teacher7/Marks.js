...
const TeacherMarks7 = () => {
  ...
  useEffect(() => {
    const fetchMarks = async () => {
      const marksCollection = collection(db, "marks7");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const fileRef = ref(storage, `marks7/${file.name}`);
      await uploadBytes(fileRef, file);
      const fileURL = await getDownloadURL(fileRef);

      const docRef = await addDoc(collection(db, "marks7"), {
        fileName,
        fileURL,
        uploadedAt: new Date()
      });
  ...
  const handleDelete = async (id, fileURL) => {
    ...
    try {
        ...
        await deleteDoc(doc(db, "marks7", id)); // Corrected collection name
