...
const TeacherMarks6 = () => {
  ...
  useEffect(() => {
    const fetchMarks = async () => {
      const marksCollection = collection(db, "marks6");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const fileRef = ref(storage, `marks6/${file.name}`);
      await uploadBytes(fileRef, file);
      const fileURL = await getDownloadURL(fileRef);

      const docRef = await addDoc(collection(db, "marks6"), {
        fileName,
        fileURL,
        uploadedAt: new Date()
      });
  ...
  const handleDelete = async (id, fileURL) => {
    ...
    try {
        ...
        await deleteDoc(doc(db, "marks6", id)); // Corrected collection name
...
return (
    <div className="container mx-auto px-4 py-8">
        <h2 className="text-3xl font-bold mb-6">Upload Marks - Grade 6</h2>
