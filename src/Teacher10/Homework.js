...
const TeacherHomework10 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const homeworkSnapshot = await getDocs(collection(db, 'homework10')); // Updated collection name for Grade 10
        ...
        const submissionSnapshot = await getDocs(collection(db, 'studenthomework10')); // Updated collection name for submissions
  ...
  const handleAddHomework = async (e) => {
    ...
    try {
      ...
      setUploading(true);
      if (file) {
        const fileRef = ref(storage, `homework10/${file.name}`); // Updated file path for Grade 10
      ...
      const docRef = await addDoc(collection(db, 'homework10'), newHomework); // Updated collection name for Grade 10
  ...
  const handleDeleteHomework = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this homework?')) {
      try {
        ...
        await deleteDoc(doc(db, 'homework10', id)); // Updated collection name for Grade 10
  ...
  const handleDeleteSubmission = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this submission?')) {
      try {
        await deleteDoc(doc(db, 'studenthomework10', id)); // Updated collection name for submissions

        if (fileURL) {
          const fileRef = ref(storage, `studenthomework10/${fileURL.split('/').pop()}`);
          await deleteObject(fileRef);
        }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Homework - Grade 10</h2> {/* Updated title for Grade 10 */}
...
export default TeacherHomework10;
