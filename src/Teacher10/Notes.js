...
const TeacherNotesGrade10 = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      setLoading(true);
      try {
        const querySnapshot = await getDocs(collection(db, 'notes10'));
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'notes10'), newNote);
  ...
  const grade10Notes = notesList.filter(note => note.grade === '10');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 p-10">
      <header className="mb-10">
        <h2 className="text-gray-800 text-2xl mb-2">Teacher's Notes - Grade 10</h2>
      </header>
      ...
      {/* Render grade 10 notes */}
      <div>
        <h3 className="text-2xl font-semibold mb-4">Grade 10 Notes</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {grade10Notes.map(note => (
...
export default TeacherNotesGrade10;
