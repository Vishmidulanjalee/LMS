...
const February = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const q = query(collection(db, 'recordings'), where('month', '==', 'February'), orderBy('timestamp', 'asc'));
        ...
        console.error('Error fetching February recordings:', err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: '#111827', margin: 0, lineHeight: 1.2 }}>
                Recordings — February 2026
              </h1>
              <p style={{ fontSize: 13, color: '#9CA3AF', margin: '2px 0 0', fontWeight: 400 }}>Class recordings for February</p>
            </div>
          </div>
          {!loading && totalCount > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: '#111827', borderRadius: 10, padding: '9px 16px' }}>
              <VideoIcon />
              <span style={{ fontSize: 13, color: 'white', fontWeight: 600 }}>{totalCount} {totalCount === 1 ? 'recording' : 'recordings'}</span>
            </div>
          )}
        </div>
      </header>
      ...
      <main style={{ flexGrow: 1, padding: '30px 32px', maxWidth: 1280, margin: '0 auto', width: '100%' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 14 }}>
            ...
            <p style={{ fontSize: 15, color: '#6B7280', margin: 0, fontWeight: 500 }}>No recordings found for February.</p>
          </div>
        ) : (
          Object.entries(groupedRecordings).map(([type, recs], idx) => (
            <div key={idx} style={{ marginBottom: 36 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                ...
                <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 19, fontWeight: 700, color: '#111827', margin: 0 }}>{type}</h2>
