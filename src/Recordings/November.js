...
const November = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const q = query(collection(db, 'recordings'), where('month', '==', 'November'), orderBy('timestamp', 'asc'));
        const snap = await getDocs(q);
        setRecordings(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      } catch (err) {
        console.error('Error fetching November recordings:', err);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div style={{ padding: '15px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', maxWidth: 1280, margin: '0 auto', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: '#9CA3AF', margin: '2px 0 0', fontWeight: 400 }}>Class recordings for November</p>
            </div>
          </div>
      ...
      <main style={{ flexGrow: 1, padding: '30px 32px', maxWidth: 1280, margin: '0 auto', width: '100%' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 14 }}>
            ...
            <p style={{ fontSize: 15, color: '#6B7280', margin: 0, fontWeight: 500 }}>No recordings found for November.</p>
          </div>
        ) : (
