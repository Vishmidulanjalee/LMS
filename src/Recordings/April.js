  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        console.error('Error fetching April recordings:', err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: '#111827', margin: 0, lineHeight: 1.2 }}>
                Recordings — April 2026
              </h1>
              <p style={{ fontSize: 13, color: '#9CA3AF', margin: '2px 0 0', fontWeight: 400 }}>Grade 10 & 11 class recordings for April</p>
            </div>
          </div>
      ...
      <main style={{ flexGrow: 1, padding: '30px 32px', maxWidth: 1280, margin: '0 auto', width: '100%' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 14 }}>
            ...
            <p style={{ fontSize: 15, color: '#6B7280', margin: 0, fontWeight: 500 }}>No recordings found for April.</p>
          </div>
        ) : (
