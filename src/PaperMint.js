...
const PaperMint = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const q = query(collection(db, 'documents'), where('category', '==', 'Paper Mint Papers'));
      ...
      <header style={{ background: 'white', borderBottom: '1px solid #E5E7EB', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ padding: '15px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', maxWidth: 1280, margin: '0 auto', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ width:46,height:46,borderRadius:13,background:'linear-gradient(135deg,#FBBF24,#D97706)',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 16px rgba(245,158,11,0.38)',flexShrink:0 }}>
              <FileIcon />
            </div>
            <div>
              <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:'#111827',margin:0,lineHeight:1.2 }}>Paper Mint Papers</h1>
              <p style={{ fontSize:13,color:'#9CA3AF',margin:'3px 0 0',fontWeight:400 }}>Class practice papers</p>
            </div>
          </div>
          {!loading && documents.length > 0 && (
            <div style={{ display:'flex',alignItems:'center',gap:7,background:'#111827',borderRadius:10,padding:'9px 16px' }}>
              <FileIcon />
              <span style={{ fontSize:13.5,color:'white',fontWeight:600 }}>{documents.length} {documents.length===1?'document':'documents'}</span>
            </div>
          )}
        </div>
      </header>
      ...
      <main style={{ flexGrow:1,padding:'30px 32px',maxWidth:1280,margin:'0 auto',width:'100%' }}>
        ...
        {!loading && documents.length === 0 && (
          <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'80px 0',gap:14 }}>
            ...
            <p style={{ fontSize:13,color:'#9CA3AF',margin:0 }}>Check back later for new Paper Mint Papers.</p>
          </div>
        )}
        {!loading && documents.length > 0 && (
