...
const TeacherHomework7 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const homeworkSnapshot = await getDocs(collection(db, 'homework7'));
        ...
        const submissionSnapshot = await getDocs(collection(db, 'studenthomework7'));
  ...
  const handleAddHomework = async (e) => {
    ...
    try {
      ...
      setUploading(true);
      if (file) {
        const fileRef = ref(storage, `homework7/${file.name}`);
      ...
      const docRef = await addDoc(collection(db, 'homework7'), newHomework);
  ...
  const handleDeleteHomework = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this homework?')) {
      try {
        ...
        await deleteDoc(doc(db, 'homework7', id));
  ...
  const handleDeleteSubmission = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this submission?')) {
      try {
        await deleteDoc(doc(db, 'studenthomework7', id));

        if (fileURL) {
          const fileRef = ref(storage, `studenthomework7/${fileURL.split('/').pop()}`);
          await deleteObject(fileRef);
        }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Homework - Grade 7</h2>
...
export default TeacherHomework7;