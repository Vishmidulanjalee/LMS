...
const TeacherMarks9 = () => {
  ...
  useEffect(() => {
    const fetchMarks = async () => {
      const marksCollection = collection(db, "marks9"); // Updated collection for Grade 9
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const fileRef = ref(storage, `marks9/${file.name}`); // Updated storage path for Grade 9
      await uploadBytes(fileRef, file);
      const fileURL = await getDownloadURL(fileRef);

      const docRef = await addDoc(collection(db, "marks9"), { // Updated collection name for Grade 9
        fileName,
        fileURL,
        uploadedAt: new Date()
      });
  ...
  const handleDelete = async (id, fileURL) => {
    try {
      ...
      await deleteDoc(doc(db, "marks9", id)); // Corrected collection name
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Upload Marks - Grade 9</h2>
