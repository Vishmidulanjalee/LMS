...
const Submission11 = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `studenthomework11/${fileType}/${file.name}`); // Updated storage path for Grade 11
      await uploadBytes(storageRef, file);

      const fileURL = await getDownloadURL(storageRef);

      // Save to Firestore
      await addDoc(collection(db, 'studenthomework11'), { // Updated collection name for Grade 11
        name,
        fileURL,
        className,
        institute,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Submit Your Homework - Grade 11</h2>
