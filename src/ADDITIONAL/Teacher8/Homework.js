...
const TeacherHomework8 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const homeworkSnapshot = await getDocs(collection(db, 'homework8'));
        ...
        const submissionSnapshot = await getDocs(collection(db, 'studenthomework8'));
  ...
  const handleAddHomework = async (e) => {
    ...
    try {
      ...
      setUploading(true);
      if (file) {
        const fileRef = ref(storage, `homework8/${file.name}`);
      ...
      const docRef = await addDoc(collection(db, 'homework8'), newHomework);
  ...
  const handleDeleteHomework = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this homework?')) {
      try {
        ...
        await deleteDoc(doc(db, 'homework8', id));
  ...
  const handleDeleteSubmission = async (id, fileURL) => {
    if (window.confirm('Are you sure you want to delete this submission?')) {
      try {
        await deleteDoc(doc(db, 'studenthomework8', id));

        if (fileURL) {
          const fileRef = ref(storage, `studenthomework8/${fileURL.split('/').pop()}`);
          await deleteObject(fileRef);
        }
...
export default TeacherHomework8;