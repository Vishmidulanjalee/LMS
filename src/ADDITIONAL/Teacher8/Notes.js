...
const TeacherNotesGrade8 = () => {
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'notes8'), newNote);
  ...
  const groupedNotesByInstitution = grade8Notes.reduce((acc, note) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Upload Notes - Grade 8</h2>
      ...
      <div className="mb-8">
        ...
        <p>No notes available for Grade 8.</p>
      )}
      </div>
    </div>
  );
