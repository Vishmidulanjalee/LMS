...
const TeacherNotesGrade11 = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      setLoading(true);
      try {
        const querySnapshot = await getDocs(collection(db, 'notes11'));
  ...
  const groupedNotesByInstitution = grade11Notes.reduce((acc, note) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
            <h2 className="text-3xl font-bold mb-6">Upload Notes - Grade 11</h2>
      ...
      Object.entries(groupedNotesByInstitution).map(([institutionName, notes]) => (
        ...
        <p>No notes available for Grade 11.</p>
      )}
    </div>
    </div>
  );
